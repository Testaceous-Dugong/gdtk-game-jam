...
func _ready() -> void:
  ...
  GlobalMessageBus.level_changed.connect(set_level_changing.unbind(1).bind(true))
...
func set_level_changing(value: bool) -> void:
  level_changing = value

func advance_turn() -> void:
  if level_changing:
    print("skipping turn advance because level is changing")
    return
